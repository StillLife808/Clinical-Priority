...
ggplot(prism_std, aes(x=PRISM)) +
  ...
  geom_point(aes(x=as.numeric(PRISM), y=std*coeff)) +
  scale_y_continuous(
    # Features of the first axis
    name = "log(Acuity)",
    # Add a second axis and specify its features
...
# ------------------------------------------------------------------------------
#               Calculate p-value for Variance/Sd vs. PRISM
# ------------------------------------------------------------------------------
# use linear regression model to find r squared and p-value of
# standard deviation vs. PRISM
m <- lm(std ~ as.numeric(PRISM), prism_std)
summary(m)$r.squared
summary(m)

# use linear regression model to find r squared and p-value of
# variance vs. PRISM
p <- lm(variance ~ as.numeric(PRISM), prism_var)
summary(p)$r.squared
# get p-value for slope significance
summary(p)
